...
/**
 ...
 SAOFB-UW is free software: you can redistribute it and/or modify
 it under the terms of the GNU General Public License as published by
 the Free Software Foundation, either version 3 of the License, or
 (at your option) any later version.

 SAOFB-UW is distributed in the hope that it will be useful,
...
void readConfig()
{
    ...
    useCustomFPSCap = config.ReadBoolean("Framerate", "useCustomFPSCap", true);
    maxFPS = config.ReadInteger("Framerate", "maxFPS", 60);
}

void fovCalc()
{
    // Declare the vertical and horizontal resolution variables.
    int hRes = *(int*)((intptr_t)baseModule + 0x4BE4AA0); // Grabs Horizontal Resolution integer.
    int vRes = *(int*)((intptr_t)baseModule + 0x49AEE84); // Grabs Vertical Resolution integer.
    ...
    *(float*)((intptr_t)baseModule + 0x39B3008) = FOV;
}

void pillarboxRemoval()
{
    // Writes pillarbox removal into memory ("F6 41 2C 01" to "F6 41 2C 00").
    memcpy((LPVOID)((intptr_t)baseModule + 0x1F58297), "\xF6\x41\x2C\x00", 4);
}

void uncapFPS() //Uncaps the framerate.
{
	//Writes the new t.MaxFPS cap to memory, alongside pointer.
	*(float*)(*((intptr_t*)((intptr_t)baseModule + 0x04C92448)) + 0x0) = (float)maxFPS;
}

void resolutionCheck()
{
    if (aspectRatio != (*(int*)((intptr_t)baseModule + 0x4BE4AA0) / *(int*)((intptr_t)baseModule + 0x49AEE84)))
    {
        //fovCalc();
    }
}
...
void framerateCheck()
{
	if (maxFPS != *(float*)(*((intptr_t*)((intptr_t)baseModule + 0x04C92448)) + 0x0))
	{
		uncapFPS();
	}
}
...
void StartPatch()
{
    ...
    Sleep(30000); // Sleeps the thread for five seconds before applying the memory values.
